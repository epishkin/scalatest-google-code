...
trait ParallelTestExecution extends OneInstancePerTest {
  ...
  private[scalatest] def runOneTest(testName: String, reporter: Reporter, stopper: Stopper,
    ...
    runTest(testName, reporter, stopper, goodies, tracker)
  }
  ...
  protected abstract override def runTests(testName: Option[String], reporter: Reporter, stopper: Stopper, filter: Filter,
    ...
    distributor match {
      ...
      // algorithm
      case None => super.runTests(testName, reporter,stopper, filter, goodies, distributor, tracker)
      case Some(distribute) =>
        testName match {
          ...
          case Some(tn) => super.runTests(testName, reporter,stopper, filter, goodies, distributor, tracker)
          case None =>
            for (tn <- testNames) {
              val wrappedInstance =
