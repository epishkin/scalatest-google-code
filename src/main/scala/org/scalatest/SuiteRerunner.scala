  ...
  def apply(report: Reporter, stopRequested: Stopper, filter: Filter,
    ...
    try {
      ...
      try {
        ...
        report(SuiteStarting(tracker.nextOrdinal(), suite.suiteName, suite.suiteID, Some(suite.getClass.getName), formatter, Some(TopOfClass(suite.getClass.getName)), rerunnable))
        ...
        report(SuiteCompleted(tracker.nextOrdinal(), suite.suiteName, suite.suiteID, Some(suite.getClass.getName), Some(duration), formatter2, Some(TopOfClass(suite.getClass.getName)), rerunnable))
      }
      catch {
        case e: RuntimeException => {
          ...
          val duration = System.currentTimeMillis - suiteStartTime
          report(SuiteAborted(tracker.nextOrdinal(), rawString3, suite.suiteName, suite.suiteID, Some(suite.getClass.getName), Some(e), Some(duration), formatter3, Some(SeeStackDepthException), rerunnable))
        }
      }
