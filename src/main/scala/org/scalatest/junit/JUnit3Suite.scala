...
private[scalatest] class MyTestListener(report: Reporter, tracker: Tracker) extends TestListener {
  ...
  def addError(testCase: Test, throwable: Throwable) {
    ...
    report(TestFailed(tracker.nextOrdinal(), getMessageGivenThrowable(throwable, false), getSuiteNameForTestCase(testCase), Some(testCase.getClass.getName), testCase.toString, testCase.toString, Some(throwable), None, Some(formatter), None))
  ...
  def addFailure(testCase: Test, assertionFailedError: AssertionFailedError) {
    ...
    val formatter = getIndentedText(testCase.toString, 1, true)
    report(TestFailed(tracker.nextOrdinal(), getMessageGivenThrowable(assertionFailedError, true), getSuiteNameForTestCase(testCase), Some(testCase.getClass.getName), testCase.toString, testCase.toString, Some(assertionFailedError), None, Some(formatter), None))
  ...
  def endTest(testCase: Test) {
    ...
    if (!testHadFailed) {
      if (testCase == null)
        throw new NullPointerException("testCase was null")
      val formatter = getIndentedText(testCase.toString, 1, true)
      report(TestSucceeded(tracker.nextOrdinal(), getSuiteNameForTestCase(testCase), Some(testCase.getClass.getName), testCase.toString, testCase.toString, None, Some(formatter), None))
    }
    else {
      failedTestsSet -= testCase  
    }
