  ...
  def run() {
    ...
    if (!stopRequested()) {
      ...
      if (!suite.isInstanceOf[DistributedTestRunnerSuite])
        dispatch(SuiteStarting(tracker.nextOrdinal(), suite.suiteName, suite.suiteID, Some(suite.getClass.getName), suite.decodedSuiteName, formatter, Some(TopOfClass(suite.getClass.getName)), rerunnable))
      ...
      try {
        ...
        if (!suite.isInstanceOf[DistributedTestRunnerSuite])
          dispatch(SuiteCompleted(tracker.nextOrdinal(), suite.suiteName, suite.suiteID, Some(suite.getClass.getName), suite.decodedSuiteName, Some(duration), formatter, Some(TopOfClass(suite.getClass.getName)), rerunnable))
      }
      catch {
        case e: RuntimeException => { // Do fire SuiteAborted even if a DistributedTestRunnerSuite 
          ...
          val duration = System.currentTimeMillis - suiteStartTime
          dispatch(SuiteAborted(tracker.nextOrdinal(), rawString3, suite.suiteName, suite.suiteID, Some(suite.getClass.getName), suite.decodedSuiteName, Some(e), Some(duration), formatter3, Some(SeeStackDepthException), rerunnable))
        }
      }
    }
  }
