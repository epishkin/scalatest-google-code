...
class StartWithSubstringSpec extends Spec with ShouldMatchers with Checkers with ReturnsNormallyThrowsAssertion {
  ...
  describe("The startWith substring syntax") {
    ...
    it("should throw TestFailedException if the string does not match substring specified as a string") {
      ...
      check((s: String) => s.indexOf("1.7") == -1 ==> throwsTestFailedException(s should startWith substring ("1.7")))
    }
    ...
    it("should throw TestFailedException if the string does matches substring specified as a string when used with not") {
      ...
      check((s: String) => s.length != 0 ==> throwsTestFailedException(s should not startWith substring (s.substring(0, 1))))
    }
    ...
    it("should throw TestFailedException if the string starts with the specified substring when used in a logical-and expression") {
      ...
      check((s: String, t: String, u: String) => (s + u).indexOf(t) != 0 ==> throwsTestFailedException(s + u should (startWith substring (s) and startWith substring (t))))
    }
      ...
      check(
        (s: String, t: String, u: String, v: String) => {
          (t.length != 0 && v.length != 0 && (s + u).indexOf(t) != 0 && (s + u).indexOf(v) != 0) ==>
            throwsTestFailedException(s + u should (startWith substring (t) or startWith substring (v)))
        }
      )
    }
    ...
    it("should throw TestFailedException if the string starts with the specified substring when used in a logical-and expression used with not") {
      ...
      check(
        (s: String, t: String, u: String) =>
          (s + t + u).indexOf("hi") != 0 ==>
            throwsTestFailedException(s + t + u should (not startWith substring ("hi") and not startWith substring (s)))
      )
    }
    ...
    it("should throw TestFailedException if the string starts with the specified substring when used in a logical-or expression used with not") {
      ...
      check(
        (s: String, t: String) =>
          throwsTestFailedException(s + t should (not startWith substring (s) or not startWith substring ("")))
      )
    }
  }
}
