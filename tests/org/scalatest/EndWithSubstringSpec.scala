...
class EndWithSubstringSpec extends Spec with ShouldMatchers with Checkers with ReturnsNormallyThrowsAssertion {
  ...
  describe("The endWith substring syntax") {
    ...
    it("should throw TestFailedException if the string does not match the specified substring") {
      ...
      check((s: String) => !(s endsWith "1.7") ==> throwsTestFailedException(s should endWith substring ("1.7")))
    }
    ...
    it("should throw TestFailedException if the string does matches the specified substring when used with not") {
      ...
      check((s: String) => s.length != 0 ==> throwsTestFailedException(s should not endWith substring (s.substring(s.length - 1, s.length))))
    }
    ...
    it("should throw TestFailedException if the string ends with the specified substring when used in a logical-and expression") {
      ...
      check((s: String, t: String, u: String) => !((s + u) endsWith t) ==> throwsTestFailedException(s + u should (endWith substring (u) and endWith substring (t))))
    }
      ...
      check(
        (s: String, t: String, u: String, v: String) => {
          (t.length != 0 && v.length != 0 && !(s + u).endsWith(t) && !(s + u).endsWith(v)) ==>
            throwsTestFailedException(s + u should (endWith substring (t) or endWith substring (v)))
        }
      )
    }
    ...
    it("should throw TestFailedException if the string ends with the specified substring when used in a logical-and expression used with not") {
      ...
      check(
        (s: String, t: String, u: String) =>
          (s + t + u).indexOf("hi") != 0 ==>
            throwsTestFailedException(s + t + u should (not endWith substring ("hi") and not endWith substring (u)))
      )
    }
    ...
    it("should throw TestFailedException if the string ends with the specified substring when used in a logical-or expression used with not") {
      ...
      check(
        (s: String, t: String) =>
          throwsTestFailedException(s + t should (not endWith substring (t) or not endWith substring ("")))
      )
    }
  }
}
